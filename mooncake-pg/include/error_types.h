...
inline PGError addPGErrorContext(PGError error, std::string_view expression,
    ...
    // Keep the root cause first: the public C API stores only the leading
    // MOONCAKE_PG_MAX_ERROR_STRING bytes of this message. Context is appended
    // from the innermost propagation site to the outermost one.
    const auto separator = file.find_last_of("/\\");
    if (separator != std::string_view::npos) {
        file.remove_prefix(separator + 1);
    }

    // Context is diagnostic enrichment, so an allocation failure while
    // constructing it must not replace the original error.
    try {
        std::string context("\n  while ");
