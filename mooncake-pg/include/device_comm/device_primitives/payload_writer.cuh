...
// local region. On a non-direct path, the calling GPU kernel writes the payload
// directly into this buffer, and publish() passes region_offset as the source
...
class PayloadWriteView {
    ...
    [[nodiscard]] __device__ __forceinline__ TransferTicket publish(
        ...
        switch (path_) {
            case PayloadWritePath::Direct: {
                // getPtr() currently exposes direct mappings only for P2P,
                ...
                PG_DEVICE_ASSERT(lane_.routeKind(peer_) ==
                                 DeviceRouteKind::P2p);
   ...
   private:
    ...
    __device__ __forceinline__ PayloadWriteView(
        const TransferLane& lane, GlobalRank peer, PayloadWritePath path,
        void* data, uint64_t capacity, uint64_t local_offset,
...
//
//   PayloadWriter writer(lane, peer, staging, remote_destination);
//   auto payload =
...
class PayloadWriter {
   public:
    __device__ __forceinline__ PayloadWriter(
        const TransferLane& lane, GlobalRank peer, StagingRegion staging,
        ...
        direct_remote_base_ =
            lane_.getPtr(peer_, remote_region_.region_offset);
        if (!direct_remote_base_) {
            PG_DEVICE_ASSERT(staging_.addr != nullptr);
        }
    }

    [[nodiscard]] __device__ __forceinline__ PayloadWritePath path() const {
        return direct_remote_base_ ? PayloadWritePath::Direct
                                   : PayloadWritePath::Staging;
    }
    ...
    [[nodiscard]] __device__ __forceinline__ PayloadWriteView view(
        ...
        if (selected_path == PayloadWritePath::Direct) {
            destination =
                static_cast<char*>(direct_remote_base_) + remote_offset;
        } else {
            ...
            destination =
                static_cast<char*>(staging_.addr) + staging_offset;
            local_offset = staging_.region_offset + staging_offset;
        }

        return PayloadWriteView(
            lane_, peer_, selected_path, destination, capacity, local_offset,
   ...
   private:
    ...
    void* direct_remote_base_ = nullptr;
};
