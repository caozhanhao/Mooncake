...
// Protocol-independent lifecycle facade. It owns common resources, stream
// ordering, graph references and recovery; the selected protocol owns topology,
// publication, resource interpretation and kernel launch policy.
class DeviceCollectiveRuntime {
   ...
   private:
    ...
    int device_index_ = -1;
    std::unique_ptr<DeviceCollectiveResources> resources_;
    std::unique_ptr<FlatRingAllReduceProtocol> all_reduce_;
