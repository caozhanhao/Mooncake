...
// One algorithm-specific peer reference: the in-group rank addresses collective
// protocol state, while the remaining fields address the transfer service and
// the peer communicator's control slice.
struct DeviceCollectivePeerTarget {
    InGroupRank rank = kInvalidInGroupRank;
