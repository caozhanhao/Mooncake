...
class TransferLane {
   public:
    // Every thread in `block` must call these operations with the same request
    // values. Waiting is optional, but every thread must enter ticket.wait()
    // together when the caller needs local completion.
    __device__ __forceinline__ TransferTicket
    putAndSignal(uint32_t peer_index, const PutAndSignalRequest& request,
                 cooperative_groups::thread_block block) const {
        if (!service_ || !service_->local_region || !service_->peer_routes ||
            !service_->lane_results || lane_index_ >= kTransferLaneCount ||
            peer_index >= service_->peer_capacity) {
            __trap();
        ...
        const auto& route = service_->peer_routes[peer_index];
        ...
        switch (route.kind) {
            case DeviceRouteKind::P2p:
                return TransferTicket(p2pPutAndSignal(
                    route.p2p.region_addr, source, request.remote_offset,
                    request.size, request.signal.remote_offset,
                    request.signal.delta, block));

            case DeviceRouteKind::HostProxy: {
                if (!service_->host_proxy_command_slots ||
                    route.host_proxy.region_addr == 0) {
                    __trap();
                    return TransferTicket();
                }
                ...
                return TransferTicket(hostProxyPutAndSignal(
                    ...
                    route.host_proxy.region_addr, peer_index, source,
    ...
    __device__ __forceinline__ TransferTicket
    signal(uint32_t peer_index, const SignalRequest& request,
           cooperative_groups::thread_block block) const {
        if (!service_ || !service_->peer_routes || !service_->lane_results ||
        ...
        switch (route.kind) {
            case DeviceRouteKind::P2p:
                return TransferTicket(p2pSignal(route.p2p.region_addr,
            ...
            case DeviceRouteKind::HostProxy:
                ...
                return TransferTicket(hostProxySignal(
                    service_->host_proxy_command_slots,
                    route.host_proxy.region_addr, peer_index,
                    request.signal.remote_offset, request.signal.delta,
                    request.timeout_ticks, lane_index_,
                    service_->lane_results + lane_index_, block));
    ...
    // Every thread in the lane CTA must enter waitSignal() together. The
    // Service implements only the rolling at-least predicate; callers may use
    // `observed` to enforce a stricter protocol schedule.
