...
class TransferLane {
   public:
    ...
    __device__ __forceinline__ DeviceRouteKind routeKind(GlobalRank rank) const {
        return service_->routes[rank].kind;
    }
    ...
    __device__ __forceinline__ void* getPtr(GlobalRank rank,
                                            uint64_t remote_offset) const {
        ...
        switch (route.kind) {
            ...
            case DeviceRouteKind::HostProxy: {
                ...
                return TransferTicket(hostProxyPut(
                    ...
                    source, request.remote_offset, request.size, request.signal,
