...
// Each lane owns one fixed submission slot and one result word. The
// caller decides how lanes map to its work.
...
// Terminal outcomes visible to a transfer-service caller. Invalid handles,
// malformed requests, and protocol violations are internal errors and abort
// on the device instead of being reported as peer failures.
...
// One entry in the device-resident peer table. Keep route-specific addresses
// separate: a P2P mapping is a local CUDA VA, while a HostProxy address is an
// opaque value exported by the peer service.
struct DevicePeerRoute {
...
struct DeviceTransferHandle {
    ...
    const DevicePeerRoute* peer_routes = nullptr;
    uint64_t* lane_results = nullptr;
    HostProxyCommandSlot* host_proxy_command_slots = nullptr;

    uint32_t peer_capacity = 0;

    // Return a lightweight view of one fixed service lane. This does not
    // acquire any resource; collective serialization is handled separately.
...
// payload operations. timeout_ticks is the request-wide time budget for route
// submission and optional local-completion waits; zero means no timeout.
...
// uint64_t rolling comparison. The timeout only stops this local wait; it
// never cancels a remote operation.
