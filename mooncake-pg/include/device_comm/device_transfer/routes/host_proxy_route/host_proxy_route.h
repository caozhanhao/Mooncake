...
class HostProxyRoute : public RouteProvider {
   public:
    ...
    HostProxyRoute(TransferEngine& engine, uint32_t peer_capacity);
    ...
    [[nodiscard]] PGResult<std::vector<DevicePeerRoute>> resolveRoutes(
        std::span<const std::optional<DeviceTransferEndpoint>> peers) override;
