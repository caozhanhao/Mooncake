...
struct HostProxyCommand {
    ...
    uint32_t target_peer_index = UINT32_MAX;
};
...
//   acquire completed_sequence = N  <--  write result
//                                        release completed_sequence = N
//   read result
//
// The service contract permits only one producer per lane at a time. Keep
// this type as plain shared ABI data; host and device synchronization helpers
// live in their respective implementation files because they use different
// atomic primitives.
struct alignas(64) HostProxyCommandSlot {
