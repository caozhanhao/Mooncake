...
__device__ __forceinline__ void addP2pSignal(
    ...
    if (block.thread_rank() == 0) {
        ...
        // Each counter has one publisher in a transfer generation, so a
        // remote acquire-load followed by a release-store is sufficient. A
        // future multi-publisher primitive would require remote atomic add.
...
p2pPutAndSignal(uint64_t remote_region_address, const void* source,
                uint64_t remote_payload_offset, uint64_t size,
                uint64_t remote_signal_offset, uint64_t signal_delta,
                cooperative_groups::thread_block block) {
    if (remote_region_address == 0) {
        __trap();
        return {};
    }
    auto* const remote_region =
        reinterpret_cast<char*>(static_cast<uintptr_t>(remote_region_address));
    if (size != 0) {
        copyP2pPayload(remote_region + remote_payload_offset, source, size,
                       block);
    }
    addP2pSignal(remote_region, remote_signal_offset, signal_delta, block);
    return {};
}

__device__ __forceinline__ P2pTransferTicket
p2pSignal(uint64_t remote_region_address, uint64_t remote_signal_offset,
