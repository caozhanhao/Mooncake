...
class RouteProvider {
   public:
    ...
    // Resolve the service-owned peer endpoint snapshot as one batch.
    ...
    [[nodiscard]] virtual PGResult<std::vector<DevicePeerRoute>> resolveRoutes(
        std::span<const std::optional<DeviceTransferEndpoint>> peers) = 0;
   ...
   protected:
    [[nodiscard]] PGResult<const RouteEndpoint*> findEndpoint(
        const std::optional<DeviceTransferEndpoint>& peer) const {
        if (!peer) return nullptr;

        const RouteEndpoint* match = nullptr;
        for (const auto& endpoint : peer->routes) {
            if (endpoint.route_key != routeKey() ||
                endpoint.version != routeVersion()) {
                continue;
            }
            PG_ASSERT(
                !match,
