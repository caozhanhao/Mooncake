...
void CoordinatorHost::start() {
    ...
    // Use base-class (CoordinatorRpcService) function pointers so that the
    // coro_rpc function IDs match those generated by clients, which reference
    // the base class.  The impl object (rpc_impl_.get()) is still the derived
    // CoordinatorRpcServiceImpl  - virtual dispatch ensures the correct
    // override is called.
...
void CoordinatorHost::pushViewUpdate(const ViewUpdateEffect& effect) {
    ...
    auto prop_id = effect.propose_id;  // capture for ACK routing
    ...
    for (int i = 0; i < max_world_size_; ++i) {
        ...
        // agents.  Each ACK is routed to the appropriate 2PC handler:
        // always to the bootstrap handler, and to the proposal handler
        // if this push originated from a proposeViewUpdate request.
        rpc_client_->callAsync<&AgentRpcService::onViewUpdate>(
            addr, push, [this, group_id, prop_id, rank = i](ViewUpdateAck ack) {
                if (!ack.applied) return;
                // 1. Drive Bootstrap 2PC (always safe; no-op if not in
                // BootstrapSyncing).
                postBootstrapAck(group_id, rank, ack.epoch);
                // 2. Drive Proposal 2PC (if this push was from a proposal).
                if (prop_id.has_value()) {
                    postProposalAck(prop_id.value(), rank, ack.epoch,
                                    ack.applied);
                }
            });
    }
