...
void CoordinatorHost::postProposeViewUpdate(
    ...
    executor_.post(
        [this, ctx = std::move(ctx), req = std::move(req)]() mutable {
            ...
            auto result = state_machine_.handleViewUpdate(propose_id, req);
            runEffects(result.effects);
        });
}

void CoordinatorHost::onViewUpdateAck(uint64_t propose_id, GlobalRank rank,
                                      uint64_t epoch, bool applied) {
    executor_.post([this, propose_id, rank, epoch, applied]() {
        auto result = state_machine_.handleViewUpdateAck(propose_id, rank,
                                                         epoch, applied);
        runEffects(result.effects);
    });
}
