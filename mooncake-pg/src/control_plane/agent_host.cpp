...
void AgentHost::start() {
    ...
    // Use base-class (AgentRpcService) function pointers so that the coro_rpc
    // function IDs match those generated by Coordinator-side clients.
            ...
            [this, group_id](DeclareGroupResponse resp) {
                executor_.post([this, group_id, resp = std::move(resp)]() {
                    if (resp.success) {
                        ...
                        // Note: waitUntilGroupReady promises are fulfilled
                        // when a ViewUpdate with activated=true arrives
                        // (in postViewUpdate), not here.
                    } else {
                        LOG(ERROR)
...
void AgentHost::publishLocalEndpoint(GroupEndpointPublication endpoint) {
    ...
    executor_.post([this, endpoint = std::move(endpoint)]() mutable {
        ...
        LOG(INFO) << "AgentHost: sending publishEndpoint rank=" << rank_
...
void AgentHost::startRegisterRpc() {
    if (register_in_flight_) return;  // avoid duplicate registrations
...
void AgentHost::tick() {
    ...
    if (++tick_count <= 3) {
        LOG(INFO) << "AgentHost::tick() rank=" << rank_ << " conn="
                  << static_cast<int>(agent_.getCoordinatorConnection());
    }
