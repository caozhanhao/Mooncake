...
DeviceArenaSlice::~DeviceArenaSlice() noexcept { reset(); }
...
bool DeviceArenaSlice::valid() const noexcept { return owner_ != nullptr; }

void* DeviceArenaSlice::addr() const noexcept {
    if (!owner_ || !owner_->base()) return nullptr;
    return static_cast<char*>(owner_->base()) + offset_;
}
...
void DeviceArenaSlice::reset() noexcept {
...
PGResult<DeviceArenaSlice> DeviceArena::allocate(size_t size,
                                                 size_t alignment) {
    std::lock_guard<std::mutex> lock(mutex_);
    PG_VALIDATE_STATE(!closed_ && base_, "DeviceArena is closed");
    ...
    for (auto current = free_ranges_.begin(); current != free_ranges_.end();
        ...
        if (!allocation_begin || *allocation_begin < range_begin) continue;
