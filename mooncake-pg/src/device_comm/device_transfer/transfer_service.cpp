...
struct DeviceTransferService::DeviceState {
    ...
    struct DeviceMetadataLayout {
        ...
        static DeviceMetadataLayout make(uint32_t peer_capacity);
    ...
    static PGResult<std::unique_ptr<DeviceState>> create(
        ...
        auto state = std::unique_ptr<DeviceState>(
            new DeviceState(device_index, self_peer_index, peer_capacity,
        ...
        if (p2p_transport) {
            state->p2p_route = std::make_unique<P2pRoute>(
                *p2p_transport, state->region.addr(), device_index,
                self_peer_index, peer_capacity);
            state->route_providers.push_back(state->p2p_route.get());
        }
        state->host_proxy_route =
            std::make_unique<HostProxyRoute>(engine, peer_capacity);
        ...
        state->peer_endpoints[self_peer_index] = state->local_endpoint;
        ...
        // region.
        const auto layout = DeviceMetadataLayout::make(peer_capacity);
    ...
    DeviceState(int device_index, uint32_t self_peer_index,
                uint32_t peer_capacity, GpuStream route_stream,
    ...
    PGResult<void> resolveRoutes() {
        PG_VALIDATE_STATE(peer_endpoints[self_peer_index].has_value(),
        ...
        std::vector<DevicePeerRoute> selected_routes(peer_capacity);
        for (auto* provider : route_providers) {
            PG_TRY(auto candidates, provider->resolveRoutes(peer_endpoints));
            PG_VALIDATE_STATE(candidates.size() == peer_capacity,
                              "route provider returned an invalid peer table");

            for (uint32_t peer = 0; peer < peer_capacity; ++peer) {
                if (!peer_endpoints[peer]) {
                    PG_VALIDATE_STATE(
                        candidates[peer].kind == DeviceRouteKind::Unreachable,
                        "route provider returned a route for a missing peer");
                    continue;
                }
                auto& selected = selected_routes[peer];
        ...
        PG_VALIDATE_STATE(selected_routes[self_peer_index].kind !=
...
DeviceTransferService::DeviceState::DeviceMetadataLayout::make(
    ...
    layout.peer_routes_offset =
        reserveLayoutItems<DevicePeerRoute>(cursor, peer_capacity);
...
DeviceTransferService::DeviceState::DeviceMetadataLayout::bind(
    ...
    return DeviceMetadata{
        ...
        .peer_routes =
            layoutItemsAt<DevicePeerRoute>(allocation, peer_routes_offset),
        .lane_results =
...
PGResult<void> DeviceTransferService::installPeerEndpoint(
    ...
    state.peer_endpoints[peer_index] = endpoint;
    PG_TRY(state.resolveRoutes());
