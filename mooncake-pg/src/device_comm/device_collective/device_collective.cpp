...
struct AllReducePlanPublicationStaging {
    ...
    const std::array<uint64_t, kMaxDeviceCollectiveChannels>
        initial_step_sequences = {1, 1, 1, 1};
};
...
PGResult<void> DeviceCollectiveRuntime::publishAllReducePlan(
    ...
    PG_TRY_CUDA(cudaMemcpyAsync(
        kernel_resources_.control.next_step_sequences,
        publication.initial_step_sequences.data(),
        publication.initial_step_sequences.size() * sizeof(uint64_t),
...
DeviceCollectiveRuntime::~DeviceCollectiveRuntime() noexcept {
    auto result = shutdown(kDestructorShutdownTimeout);
    if (!result.has_value()) {
...
PGResult<void> DeviceCollectiveRuntime::useLocalOnly() {
    ...
    return publishAllReducePlan(DeviceAllReducePlan{
        ...
        .self_active_index = 0,
        .participant_count = 1,
        .predecessor = {.rank = self_rank_},
        .successor = {.rank = self_rank_},
    });
}
...
PGResult<void> DeviceCollectiveRuntime::materializeGroupView(
    ...
    // rank's service peer index and remote collective-control range.
    std::vector<InGroupRank> participants;
    participants.reserve(view.rank_order.size());
    for (size_t rank = 0; rank < view.rank_order.size(); ++rank) {
        const auto global_rank = view.rank_order[rank];
        ...
        PG_ASSERT(!member.isActive() || valid_target,
                  "active device collective peer has no valid control range");
        if (member.isActive()) {
            participants.push_back(static_cast<InGroupRank>(rank));
        }
    }
    ...
    auto targetFor = [&](InGroupRank rank) {
        const auto global_rank = view.rank_order[rank];
        const auto& endpoint =
            *view.members[global_rank].endpoint->device_collective;
