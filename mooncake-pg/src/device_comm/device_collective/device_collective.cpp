...
DeviceCollectiveRuntime::DeviceCollectiveRuntime(
    ...
    std::unique_ptr<DeviceCollectiveResources> resources,
...
void DeviceCollectiveRuntime::releaseHostControl() noexcept {
    if (resources_) resources_->setRecoveryMailbox(nullptr);
...
DeviceCollectiveRuntime::create(DeviceTransferService& transfer_service,
    ...
    PG_TRY(auto resources,
           DeviceCollectiveResources::create(
               arena, transfer_service.deviceHandle(), timeout_ticks,
               signal_layout.signal_count, state_layout.size,
               FlatRingPersistentStateLayout::kAlignment));
    const auto& buffer = workspace.buffer();
    PG_VALIDATE_STATE(buffer.addr(),
                      "device collective buffer address is null");
    PG_VALIDATE_STATE(buffer.size() >= kFlatRingBufferBytes,
                      "device collective buffer capacity is too small");
    const DeviceCollectiveEndpoint endpoint{
        .buffer_offset = buffer.offset(),
        .buffer_size = kFlatRingBufferBytes,
        .signal_offset = resources->signalOffset(),
        .signal_capacity = resources->signalCount(),
    };
    PG_VALIDATE_STATE(
        deviceCollectiveEndpointSupports(
            endpoint, kFlatRingBufferBytes, signal_layout.signal_count),
        "device collective endpoint resources overlap or overflow");
    ...
    auto runtime = std::unique_ptr<DeviceCollectiveRuntime>(
        new DeviceCollectiveRuntime(
            transfer_service, device_index, std::move(resources),
            strong_stream, endpoint, std::move(control_stream),
            std::move(handoff_event)));
    {
        PG_TRY(auto device_guard, GpuDeviceGuard::create(device_index));
        PG_TRY_CUDA(cudaHostAlloc(
            reinterpret_cast<void**>(&runtime->host_control_),
            sizeof(HostControl),
            cudaHostAllocMapped | cudaHostAllocPortable));
        std::construct_at(runtime->host_control_);

        void* device_control = nullptr;
        PG_TRY_CUDA(cudaHostGetDevicePointer(
            &device_control, runtime->host_control_, 0));
        runtime->resources_->setRecoveryMailbox(
            &static_cast<HostControl*>(device_control)->recovery_mailbox);
    }
    PG_TRY(runtime->all_reduce_,
           FlatRingAllReduceProtocol::create(
               transfer_service, workspace, *runtime->resources_,
               runtime->control_stream_, device_index, self_rank,
               max_group_size));
    return runtime;
}
...
PGResult<void> DeviceCollectiveRuntime::enqueueAllReduce(
    ...
    auto submitted = [&]() -> PGResult<void> {
        ...
        auto launched = all_reduce_->enqueue(
            send_buffer, recv_buffer, count, datatype, op, user_stream.get(),
            failed_ranks_hint);
...
PGResult<void> DeviceCollectiveRuntime::shutdown() {
    ...
    std::unique_ptr<DeviceCollectiveResources> resources_to_release;
    std::unique_ptr<FlatRingAllReduceProtocol> protocol_to_release;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        recovery_handler_ = {};
