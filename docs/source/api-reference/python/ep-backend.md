...
- **Mooncake PG** is a `torch.distributed` ProcessGroup backend. It registers the `mooncake` accelerator backend and
  the `mooncake-cpu` backend, implements collective and point-to-point APIs, and
  exposes dynamic-membership helpers.
...
For implementation details, see the [Mooncake PG design guide](../../design/mooncake-backend-pg.md)
...
| `pg.join_group(backend)` | Confirm ready for activation and block until real activation. | Used for scale-up, replacement, and in-place rejoin. |
...
- A joining rank starts with local-only
  collective behavior until `join_group`. The join call
  then blocks until a Coordinator-approved activation commits.
